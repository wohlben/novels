...
class GenerateChapterTestCase(TestCase):
    ...
    def setUp(self):
        ...
        chapters = [
            ...
            {'fiction': fics[0], 'url': 'someurl/chapter/334/chapter', 'content': 'blub'},
    ...
    def test_starting_data(self):
        self.assertGreater(self.monitored_novels(), 0, "test data doesn't include any monitored novels")
        self.assertGreater(self.missing_chapters(), 0, "test data isn't providing anything to add to the pending queue")

    def test_adding_fetch_events(self):
        self.add_queue_events()
        self.assertEquals(self.missing_chapters(), 0, "we're still finding chapters to queue...")
